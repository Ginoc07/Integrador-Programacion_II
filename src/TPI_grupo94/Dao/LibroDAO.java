...
private static final String SELECT_LIBRO_BY_ISBN_SQL = "SELECT l.*, f.isbn "
            + "FROM libros l "
            + "LEFT JOIN fichabibliografica f ON l.fichabibliografica_id = f.id "
            + "WHERE f.isbn = ? AND l.eliminado = FALSE";
...
// B// Buscar libros por autor de manera flexible con like
...
public List<Libro> leerTodos(Connection conn) throws Exception {
    ...
    public List<Libro> buscarPorAutor(String filtro) throws SQLException {
     ...
     public List<Libro> buscarPorIsbn(String filtro) throws SQLException {
        if (filtro == null || filtro.trim().isEmpty()) {
            throw new IllegalArgumentException("El filtro de búsqueda no puede estar vacío");
        }
        ...
        try (Connection conn = DatabaseConnection.getConnection(); PreparedStatement stmt = conn.prepareStatement(SELECT_LIBRO_BY_ISBN_SQL)) {
